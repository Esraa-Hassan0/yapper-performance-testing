...
const url = getUrl('/users'); // Base URL for user media endpoint
...
export default function (data) {
  ...
  const fullUrl = `${url}/${userId}/media?limit=20`;
  console.log(`Fetching media for user ID: ${userId}`);

  // Send GET request to fetch user media
  ...
  logStatus(res, `Fetching media for user ID: ${userId}`, 'get_user_media');

  // Validate the response
  check(res, {
    'status is 200': (r) => r.status === 200,
    'response contains media data': (r) => {
      try {
        const body = r.json();
    ...
    'response contains success message': (r) => {
      try {
        const body = r.json();
        return body.message === 'Retrieved media successfully';
      } catch {
        return false;
      }
    }
