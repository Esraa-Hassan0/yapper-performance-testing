...
const followingTimelineUrl = getUrl('/timeline/following');
...
export default function () {
  ...
  // TEST 1: Get following timeline (default - no params)
  const res1 = http.get(followingTimelineUrl, {
    headers: {
  ...
  const res2 = http.get(`${followingTimelineUrl}?limit=${limit}`, {
    headers: {
      Authorization: `Bearer ${token}`
    },
    responseCallback: http.expectedStatuses(200)
  ...
  if (cursor) {
    const res3 = http.get(`${followingTimelineUrl}?cursor=${cursor}&limit=10`, {
      headers: {
        Authorization: `Bearer ${token}`
      },
      responseCallback: http.expectedStatuses(200)
  ...
  const res4 = http.get(`${followingTimelineUrl}?limit=150`, {
    headers: {
      Authorization: `Bearer ${token}`
    },
  ...
  const res5 = http.get(followingTimelineUrl, {
    headers: {
